 ...
 This toolbox offers functions for convienient interactice im-/export of data 
 files, as comma-separated-value- (*.csv, *.dat, *.txt) and binary Excel files 
 (*.xls) to or from a Scilab matrix. 
 
 They are based on the appropriate internal Scilab functions but make the im-
 and export much more user-friendly.
 
 Parameters to specify the im-/export can convieniently entered in a dialog box 
 and the file selection is done by the platform-specific file-selection-dialog.
 ...
 * DI_writecsv
 ...
 NOTE: dataINT handles doubles only. Strings are imported as NaN (%nan).
