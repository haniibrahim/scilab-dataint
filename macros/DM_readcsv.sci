function [csvMat, exitID] = DM_readcsv(path)
    //
    // Reads a CSV file in a matrix variable interactively
    //
    // CALLING SEQUENCES
    // [csvMat, exitID] = DM_readcsv(path)
    // [csvMat, exitID] = DM_readcsv()
    //
    // PARAMETERS
    ...
    //
    // EXAMPLES
    // [csvMat, exitID] = DM_readcsv(pwd()) // Open the file selector in the currend directory
    // csvMat = DM_readcsv()
    //
    
    [lhs,rhs]=argn()
    apifun_checkrhs("DM_readcsv", rhs, 0:1); // Input args
    apifun_checklhs("DM_readcsv", lhs, 1:2); // Output args
